...
<?php 
 ...
 $conn = ConnectionFactory::getFactory("sgp_user", "56p_2016", "prueba_sgp_system")->getConnection();
  ...
  $query = "SELECT a.nombre FROM partida a INNER JOIN etapapartida b ON b.idPartida = a.numero AND b.versioNPartida = a.version INNER JOIN etapa c ON b.idEtapa = c.idEtapa INNER JOIN proyecto d ON d.idProyecto = c.idProyecto WHERE d.estado != 0 AND a.numero = ".$numeroPartida." AND a.version = ".$version." GROUP BY c.idEtapa";
  ...
  if ($res->num_rows > 0) {
    ...
    $query = "INSERT INTO partida(numero, version ,nombre, totalCD, totalCF, precioUnitario, totalMateriales, totalManoObra, totalEquipoHerramientas, totalSubContratos) VALUES (".$numeroPartida.", ".$nuevaVersion.", '".$nombre."', ".$cd.", ".$porcentaje_indirecto.", ".$precio_unitario.", ".$total_materiales.", ".$total_MO.", ".$total_herramientas.", ".$total_subcontratos.")";
      ...
      for ($i = 0; $i < $n; $i++){
          $query = "INSERT INTO lineaequipoherramientaPartida (idLinea, numPartida, versionPartida) VALUES (".$_POST["idLineaMO"][$i].", ".$numeroPartida.", ".$nuevaVersion.")";
          $conn->query($query);
      }
      $offsetMO = $i;      
  ...
  // Creamos los nuevas lineas agregadaspara cualquier caso
    ...
    for ($i = 0; $i < $n; $i++){
      $query = "INSERT INTO linearecurso (codigo, version, cantidad, subTotal) VALUES(".$_POST["codigo"][$i].", ".$_POST["version"][$i]." ,".$_POST["cantidadMateria"][$i+$offsetMatPrima].", ".$_POST["subTotal_recursos"][$i+$offsetMatPrima].")";
      $conn->query($query);
      $idLinea = $conn->insert_id;
      $query = "INSERT INTO linearecursoPartida (idLinea, numPartida, versionPartida) VALUES (".$idLinea.", ".$numeroPartida.", ".$version.")";
          $conn->query($query);
    }
    ...
    $n = count($_POST["rendimientoMO"]) - count($_POST["idLineaMO"]);
      for ($i = 0; $i < $n; $i++){
        ...
        $conn->query($query);
        $idLinea = $conn->insert_id;
          $query = "INSERT INTO lineamanoobraPartida (idLinea, numPartida, versionPartida) VALUES (".$idLinea.", ".$numeroPartida.", ".$version.")";
          $conn->query($query);
      }
      ...
      for ($i = 0; $i < $n; $i++){
        ...
        $conn->query($query);
         $idLinea = $conn->insert_id;
          $query = "INSERT INTO lineaequipoherramientaPartida (idLinea, numPartida, versionPartida) VALUES (".$idLinea.", ".$numeroPartida.", ".$version.")";
          $conn->query($query);
      }
